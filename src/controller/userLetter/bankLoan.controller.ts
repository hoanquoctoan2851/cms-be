...
export async function createBankLoanLetter(
  ...
  try {
    ...
    const newLetter = await BankLoanModel.create({
      ...req.body,
...
export async function getBankLoanLetterByUserId(
...
export async function detailBankLoanLetter(
  req: Request<RequestParams>,
  res: Response
) {
  try {
    const { id } = req.params;
    const data = await BankLoanModel.findById(id);
    return res.send({
      success: true,
      data: data,
    });
...
export async function updateBankLoanLetter(
  req: Request<{ id: string }, {}, createBankLoanInput>,
  res: Response
) {
  const { id } = req.params;
  const letter = await BankLoanModel.findByIdAndUpdate(id, req.body, {
    new: true,
    upsert: true,
  });
...
export async function deleteBankLoanLetter(
  req: Request<{ id: string }>,
  res: Response
) {
  const { id } = req.params;
  try {
    const letter = await BankLoanModel.findById(id);
    if (!letter) {
      return res.send({
        success: false,
