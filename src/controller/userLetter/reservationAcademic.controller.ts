...
import { createReservationAcademicSchemaInput } from "./../../schema/letters/reservationAcademic.schema";

export async function createReservationAcademicLetter(
  req: Request<{}, {}, createReservationAcademicSchemaInput>,
  res: Response
) {
  try {
    ...
    const newLetter = await ReservationAcademicModel.create({
      ...req.body,
...
export async function getReservationAcademicByUserId(
...
export async function detailReservationAcademicLetter(
  req: Request<RequestParams>,
  res: Response
) {
  try {
    const { id } = req.params;
    const data = await ReservationAcademicModel.findById(id);
    return res.send({
      success: true,
      data: data,
    });
...
export async function updateReservationAcademicLetter(
  req: Request<{ id: string }, {}, createReservationAcademicSchemaInput>,
  res: Response
) {
  const { id } = req.params;
  const letter = await ReservationAcademicModel.findByIdAndUpdate(
    id,
    req.body,
    {
      new: true,
...
export async function deleteReservationAcademicLetter(
  req: Request<{ id: string }>,
  res: Response
) {
  const { id } = req.params;
  try {
    const letter = await ReservationAcademicModel.findById(id);
    if (!letter) {
      return res.send({
        success: false,
