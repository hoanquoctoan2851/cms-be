...
import { createCancelCourseInput } from "./../../schema/letters/cancelCourse.schema";
import { decreaseUserBalance } from "../../middleware/user/changeBalanceUser";
import BankLoanModel from "../../model/Letters/bankLoan.model";

// Đơn hủy học phần

export async function createCancelCourseLetter(
  req: Request<{}, {}, createCancelCourseInput>,
  res: Response
) {
  try {
    ...
    const newLetter = await CancelCourseModel.create({
      ...req.body,
    ...
    if (newLetter) {
      ...
      if(isBalanceDecreased) {
        ...
        await CancelCourseModel.findByIdAndDelete(newLetter._id);
...
export async function getCancelCourseLetterByUserId(
...
export async function detailCancelCourseLetter(
  req: Request<RequestParams>,
  res: Response
) {
  try {
    const { id } = req.params;
    const data = await CancelCourseModel.findById(id);
    return res.send({
      success: true,
      data: data,
    });
...
export async function updateCancelCourseLetter(
  req: Request<{ id: string }, {}, createCancelCourseInput>,
  res: Response
) {
  const { id } = req.params;
  const letter = await CancelCourseModel.findByIdAndUpdate(id, req.body, {
    new: true,
    upsert: true,
  });
...
export async function deleteConfirmStudyingLetter(
  ...
  const { id } = req.params;
  try {
    const letter = await CancelCourseModel.findById(id);
    if (!letter) {
      return res.send({
        success: false,
