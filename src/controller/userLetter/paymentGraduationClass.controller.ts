...
import { createPaymentGraduationClassSchemaInput } from "./../../schema/letters/paymentGraduationClass.shema";

// Đơn hủy học phần

export async function createPaymentGraduationClass(
  req: Request<{}, {}, createPaymentGraduationClassSchemaInput>,
  res: Response
) {
  try {
    ...
    const newLetter = await PaymentGraduationClassModel.create({
      ...req.body,
...
export async function getPaymentGraduationClassLetterByUserId(
...
export async function detailPaymentGraduationClassLetter(
  req: Request<RequestParams>,
  res: Response
) {
  try {
    const { id } = req.params;
    const data = await PaymentGraduationClassModel.findById(id);
    return res.send({
      success: true,
      data: data,
    });
...
export async function updatePaymentGraduationClassLetter(
  req: Request<{ id: string }, {}, createPaymentGraduationClassSchemaInput>,
  res: Response
) {
  const { id } = req.params;
  const letter = await PaymentGraduationClassModel.findByIdAndUpdate(
    id,
    req.body,
    {
      new: true,
...
export async function deletePaymentGraduationClassLetter(
  req: Request<{ id: string }>,
  res: Response
) {
  const { id } = req.params;
  try {
    const letter = await PaymentGraduationClassModel.findById(id);
    if (!letter) {
      return res.send({
        success: false,
