...
import { createRenewStudentCardSchemaInput } from "./../../schema/letters/renewStudentCard.schema";

export async function createRenewStudentCardLetter(
  req: Request<{}, {}, createRenewStudentCardSchemaInput>,
  res: Response
) {
  try {
    ...
    const newLetter = await RenewStudentCardModel.create({
      ...req.body,
...
export async function getRenewStudentCardLetterByUserId(
...
export async function detailRenewStudentCardLetter(
  req: Request<RequestParams>,
  res: Response
) {
  try {
    const { id } = req.params;
    const data = await RenewStudentCardModel.findById(id);
    return res.send({
      success: true,
      data: data,
    });
...
export async function updateRenewStudentCardLetter(
  req: Request<{ id: string }, {}, createRenewStudentCardSchemaInput>,
  res: Response
) {
  const { id } = req.params;
  const letter = await RenewStudentCardModel.findByIdAndUpdate(id, req.body, {
    new: true,
    upsert: true,
  });
...
export async function deleteRenewStudentCardLetter(
  req: Request<{ id: string }>,
  res: Response
) {
  const { id } = req.params;
  try {
    const letter = await RenewStudentCardModel.findById(id);
    if (!letter) {
      return res.send({
        success: false,
