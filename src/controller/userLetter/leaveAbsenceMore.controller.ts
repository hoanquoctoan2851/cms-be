...
export async function createLeaveAbsenceMoreLetter(
  ...
  try {
    ...
    const newLetter = await LeaveAbsenceMoreModel.create({
      ...req.body,
...
export async function getLeaveAbsenceMoreLetterByUserId(
...
export async function detailLeaveAbsenceMoreLetter(
  req: Request<RequestParams>,
  res: Response
) {
  try {
    const { id } = req.params;
    const data = await LeaveAbsenceMoreModel.findById(id);
    return res.send({
      success: true,
      data: data,
    });
...
export async function updateLeaveAbsenceMoreLetter(
  req: Request<{ id: string }, {}, createLeaveAbsenceLessSchemaInput>,
  res: Response
) {
  const { id } = req.params;
  const letter = await LeaveAbsenceMoreModel.findByIdAndUpdate(id, req.body, {
    new: true,
    upsert: true,
  });
...
export async function deleteLeaveAbsenceMoreLetter(
  req: Request<{ id: string }>,
  res: Response
) {
  const { id } = req.params;
  try {
    const letter = await LeaveAbsenceMoreModel.findById(id);
    if (!letter) {
      return res.send({
        success: false,
